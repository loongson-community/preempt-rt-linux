/*
 * gdium-laptop  --  Gdium laptop extras
 *
...
static unsigned char gpio16 = 0;
static unsigned char ec = 0;
		...
		union power_supply_propval *val)
{
	...
	if (!data) {
		printk(KERN_ERR "gdium-ac: gdium_laptop_data not found\n");
		return -EINVAL;
	}
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_ONLINE:
			val->intval = status & EC_STATUS_ADAPT ? 1 : 0;
	                break;
		default:
			ret = -EINVAL;
			break;
		...
		union power_supply_propval *val)
{
	...
	if (!data) {
		printk(KERN_ERR "gdium-battery: gdium_laptop_data not found\n");
		return -EINVAL;
	}
...
static void gdium_laptop_battery_work(struct work_struct *work)
{
	...
	if ((data->status & EC_STATUS_ADAPT) && present && (data->battery_level <= BAT_MAX))
		if (ret < BAT_TRICKLE_EN)
			ec_charge_en(client, 1, 0);
		else
			ec_charge_en(client, 1, 1);
	else
		ec_charge_en(client, 0, 0);
...
static int gdium_laptop_probe(struct i2c_client *client, 
	const struct i2c_device_id *dev_id)
{
	...
	data->debugfs = debugfs_create_file("gdium-laptop", S_IFREG | S_IRUGO,
