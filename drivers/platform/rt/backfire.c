...
DEFINE_SPINLOCK(backfire_state_lock);
...
static int backfire_open(struct inode *inode, struct file *file)
{
...
static int backfire_release(struct inode *inode, struct file *file)
{
	spin_lock(&backfire_state_lock);
...
static int __init backfire_init(void)
{
	...
	if (ret)
		printk(KERN_ERR
		       "backfire: can't register dynamic misc device\n");
	else
		printk(KERN_INFO "backfire driver v" VERSION
		       " misc device %d\n", backfire_dev.minor);
	return ret;
}
