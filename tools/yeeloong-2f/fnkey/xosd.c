...
#define SLIDER_SCALE 2
#define SLIDER_SCALE_ON 0.9
#define XOFFSET 1 

const char *osd_default_font = "Sans-12";
...
/* Draw Icon */
static void
_draw_icon(xosd * osd, int on, XRectangle * p, XRectangle * mod)
{
  int i, j, k;
  int count;
  XPoint *point = NULL;
  ...
  xosd_icon_element *current;

  FUNCTION_START(Dfunction);

  for (i = 0; i < sizeof(osd_icon)/sizeof(xosd_icon); i++) {
	  if (osd_icon[i].type == osd->type)
		  break;
  }
  if (osd_icon[i].type == XOSD_null) {
	  printf("Error, cannot find type %d\n", osd->type);
	  return;
  }
  for (j = 0; j < osd_icon[i].nstage; j++){
	  if ((osd_icon[i].element)[j].bar <= on)
		  break; /* found bar */
  }
  if (j >= osd_icon[i].nstage)
	  j = osd_icon[i].nstage - 1;

  /* recaculate points, arcs and rectangles */
  current = &((osd_icon[i].element)[j]);
  for (k = 0; k < ATOM_TYPE_max; k++) {
	  int n = 0;
	  while (current->atom[k].x_atom != NULL && current->atom[k].x_atom[n].member.list) {
		  for (count = 0; current->atom[k].x_atom[n].member.list[count].x != -1; count++);
		  switch (current->atom[k].atom_type) {
			  case  ATOM_TYPE_rectangle:
				  rect = malloc(sizeof(XRectangle) * count);
				  for (count = 0; current->atom[k].x_atom[n].member.rect[count].x != -1; count++) {
					  rect[count].x = (current->atom[k].x_atom[n].member.rect[count].x * p->height)
						  /osd_icon[i].block + p->x;
					  rect[count].y = (current->atom[k].x_atom[n].member.rect[count].y * p->height)
						  /osd_icon[i].block + p->y;
					  rect[count].width = (current->atom[k].x_atom[n].member.rect[count].width * p->height) / osd_icon[i].block;
					  rect[count].height = (current->atom[k].x_atom[n].member.rect[count].height * p->height) / osd_icon[i].block;
				  }
				  if (count) {
					  if (current->atom[k].x_atom[n].is_fill) {
						  XFillRectangles(osd->display, osd->mask_bitmap, osd->mask_gc, rect, count);
						  XFillRectangles(osd->display, osd->line_bitmap, osd->gc, rect, count);
					  } else {
						  XDrawRectangles(osd->display, osd->mask_bitmap, osd->mask_gc, rect, count);
						  XDrawRectangles(osd->display, osd->line_bitmap, osd->gc, rect, count);
					  }
				  }
				  XFree(rect);
				  break;
			  case  ATOM_TYPE_point:
				  point = malloc(sizeof(XPoint) * count);
				  for (count = 0; current->atom[k].x_atom[n].member.point[count].x != -1; count++) {
					  point[count].x = (current->atom[k].x_atom[n].member.point[count].x * p->height)
						  /osd_icon[i].block + p->x;
					  point[count].y = (current->atom[k].x_atom[n].member.point[count].y * p->height)
						  /osd_icon[i].block + p->y;
				  }
				  if (count) {
					  if (current->atom[k].x_atom[n].is_fill) {
						  XFillPolygon(osd->display, osd->mask_bitmap, osd->mask_gc, point, count, Convex, CoordModeOrigin);
						  XFillPolygon(osd->display, osd->line_bitmap, osd->gc, point, count, Convex, CoordModeOrigin);
					  } else {
						  XDrawLines(osd->display, osd->mask_bitmap, osd->mask_gc, point, count, CoordModeOrigin);
						  XDrawLines(osd->display, osd->line_bitmap, osd->gc, point, count, CoordModeOrigin);
					  }
				  }
				  XFree(point);
				  break;
			  case  ATOM_TYPE_arc:
				  arc = malloc(sizeof(XArc) * count);
				  for (count = 0; current->atom[k].x_atom[n].member.arc[count].x != -1; count++) {
					  arc[count].x = (current->atom[k].x_atom[n].member.arc[count].x * p->height)
						  /osd_icon[i].block + p->x;
					  arc[count].y = (current->atom[k].x_atom[n].member.arc[count].y * p->height)
						  /osd_icon[i].block + p->y;
					  arc[count].width = (current->atom[k].x_atom[n].member.arc[count].width * p->height)
						  /osd_icon[i].block;
					  arc[count].height = (current->atom[k].x_atom[n].member.arc[count].height * p->height)
						  /osd_icon[i].block;
					  arc[count].angle1 = current->atom[k].x_atom[n].member.arc[count].angle1;
					  arc[count].angle2 = current->atom[k].x_atom[n].member.arc[count].angle2;
				  }
				  if (count) {
					  if (current->atom[k].x_atom[n].is_fill) {
						  XFillArcs(osd->display, osd->mask_bitmap, osd->mask_gc, arc, count);
						  XFillArcs(osd->display, osd->line_bitmap, osd->gc, arc, count);
					  } else {
						  XDrawArcs(osd->display, osd->mask_bitmap, osd->mask_gc, arc, count);
						  XDrawArcs(osd->display, osd->line_bitmap, osd->gc, arc, count);
					  }
				  }
				  XFree(arc);
				  break;
...
#if 0
  XDrawLines(osd->display, osd->mask_bitmap, osd->mask_gc,);
  XFillArc(osd->display, osd->mask_bitmap, osd->mask_gc, p->x, p->y, p->height, p->height, 0, 360*64);
  XFillArc(osd->display, osd->line_bitmap, osd->gc, p->x, p->y, p->height, p->height, 0, 360*64);
#endif
          ...
          int is_slider)
{
  ...
  for (i = 0; i < nbars; i++, rs[0].x = rs[1].x += p->width) {
    ...
    if (i < on) { 
	    ...
	    XDrawRectangles(osd->display, osd->mask_bitmap, osd->mask_gc, r, 1);
	    XDrawRectangles(osd->display, osd->line_bitmap, osd->gc, r, 1);
    }
#endif
  }
  FUNCTION_END(Dfunction);
...
draw_bar(xosd * osd, int line)
{
  ...
  if (osd->bar_length == -1) {
    ...
    switch (osd->align) {
    case XOSD_center:
      p.x = (osd->screen_width - (nbars * p.width) - p.height) / 2;
      break;
    case XOSD_right:
      p.x = osd->screen_width - (nbars * p.width) - p.x - p.height;
    case XOSD_left:
      break;
    }
  }
...
{
	FUNCTION_START(Dfunction);
	Display *dpy = osd->display; //be careful with XftDrawCreateBitmap, mask_bitmap's depth is 1
	XftDraw * xftDraw_mask = XftDrawCreateBitmap (dpy, osd->mask_bitmap);
	XftDraw * xftDraw_line = XftDrawCreate (dpy, osd->line_bitmap, DefaultVisual (dpy, DefaultScreen (dpy)), DefaultColormap (dpy, DefaultScreen (dpy)));

	XftColor        xftColor_mask,xftColor_line;
	xftColor_mask.color.red = 1;
	xftColor_mask.color.green = 1;
	xftColor_mask.color.blue = 1;
    	xftColor_mask.color.alpha=0xFFFF;

    	xftColor_mask.pixel = WhitePixel(dpy, DefaultScreen(dpy));
	xftColor_line.color.red = osd->colour.red;
	xftColor_line.color.green = osd->colour.green;
	xftColor_line.color.blue = osd->colour.blue;
    	xftColor_line.color.alpha=0xFFFF;

	XftDrawStringUtf8 (xftDraw_mask, &xftColor_mask, osd->font, x, y, (FcChar8 *) string, strlen (string));
	XftDrawStringUtf8 (xftDraw_line, &xftColor_line, osd->font, x, y, (FcChar8 *) string, strlen (string));

	XftColorFree (dpy, DefaultVisual (dpy, DefaultScreen (dpy)), DefaultColormap (dpy, DefaultScreen (dpy)), &xftColor_mask);
	XftColorFree (dpy, DefaultVisual (dpy, DefaultScreen (dpy)), DefaultColormap (dpy, DefaultScreen (dpy)), &xftColor_line);
...
draw_text(xosd * osd, int line)
{
  ...
  if (l->width < 0) {
	  Display *dpy = osd->display;
	  XGlyphInfo extents;
	  XftTextExtentsUtf8(dpy,osd->font,(FcChar8*)l->string, strlen(l->string), &extents);
	  l->width = extents.width;
  }
...
event_loop(void *osdv)
{
  ...
  while (!osd->done) {
	...
	XGlyphInfo xextents;
	char *test="ABC";
	XftTextExtentsUtf8(dpy, osd->font, (FcChar8*)test, strlen(test), &xextents);

	osd->extent=malloc(sizeof(osd->extent));
	osd->extent->x = xextents.x;
	osd->extent->y = xextents.yOff - xextents.y;
	osd->extent->width = xextents.width / strlen(test);
	osd->extent->height = xextents.height + 15;
...
{
  FcPattern * fpat;
  FcChar8 * family = (FcChar8 *)"Sans";
  int size=10;

  fpat = FcNameParse((FcChar8 *)font);
  if (fpat) {
//    FcPatternGetString(fpat,FC_FAMILY, 0, &family); 
    FcPatternGetInteger(fpat,FC_SIZE, 0, &size);
	...
	font2 = XftFontOpen (dpy, DefaultScreen (dpy), XFT_FAMILY, XftTypeString, family, XFT_SIZE, XftTypeInteger, size, XFT_ANTIALIAS, XftTypeBool, 1,  NULL);
  /*
	if(strstr(font,  "*"))//app using X core font style, old.
//		font2 = XftFontOpenName(dpy, DefaultScreen(dpy), osd_default_font);
	font2 = XftFontOpen (dpy, DefaultScreen (dpy), XFT_FAMILY, XftTypeString, family, XFT_SIZE, XftTypeInteger, size, XFT_ANTIALIAS, XftTypeBool, 1,  NULL);
	else
	font2 = XftFontOpen (dpy, DefaultScreen (dpy), XFT_FAMILY, XftTypeString, family, XFT_SIZE, XftTypeInteger, size, XFT_ANTIALIAS, XftTypeDouble, 12,  NULL);
//		font2 = XftFontOpenName(dpy, DefaultScreen(dpy), font);
		*/
	if (font2 == NULL) {
...
/* xosd_set_bar_height */
int 
xosd_set_bar_height(xosd *osd, int height)
{
  FUNCTION_START(Dfunction);
  if (osd == NULL)
    return -1;
  _xosd_lock(osd);
  osd->bar_height = height;
  osd->update |= UPD_size;
  _xosd_unlock(osd);
  return 0;
}
...
/* xosd_set_bar_icon */
int 
xosd_set_bar_icon(xosd *osd, xosd_icon_type type)
{
  FUNCTION_START(Dfunction);
  if (osd == NULL)
    return -1;
